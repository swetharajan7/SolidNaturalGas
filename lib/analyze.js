...
// Ultra handles the final synthesis, where reasoning quality matters most.
const ULTRA_MODEL = process.env.NEBIUS_MODEL_ULTRA || "nvidia/Nemotron-3-Ultra-550b-a55b";
// A smaller, faster model handles the short structured JSON steps
// (search planning and the evidence gap check).
const FAST_MODEL = process.env.NEBIUS_MODEL_FAST || "nvidia/Nemotron-3-Super-120B-A12B";

export function keyFor(hypothesis) {
...
// 1 hour: re-running the same hypothesis while iterating costs no credits.
const TAVILY_CACHE_TTL_SECONDS = Number(process.env.TAVILY_CACHE_TTL_SECONDS || 3600);

// "news" | "finance" | "general" — market evidence is mostly news-shaped.
// Try "finance" via the CLI and switch here if it returns better sources.
const TAVILY_TOPIC = process.env.TAVILY_TOPIC || "news";
...
const searchTavily = traceable(
  async (query) => {
    const params = { search_depth: "advanced", topic: TAVILY_TOPIC, time_range: "month" };
...
const callModelJSON = traceable(
  async (systemPrompt, userPrompt, maxTokens, model, reasoningEffort) => {
  ...
  { name: "nemotron_json_step", run_type: "llm" }
);

/*
 * Structured JSON steps run on the fast model. If that call fails
 * (bad model id, transient error, unparseable JSON), fall back to Ultra
 * so the pipeline keeps its quality instead of silently degrading.
 */
async function callFastJSON(systemPrompt, userPrompt, maxTokens) {
  try {
    return await callModelJSON(systemPrompt, userPrompt, maxTokens, FAST_MODEL, "low");
  } catch (error) {
    console.error(`Fast model (${FAST_MODEL}) failed, retrying on Ultra:`, error);
    return await callModelJSON(systemPrompt, userPrompt, maxTokens, ULTRA_MODEL, "medium");
  }
}
...
/*
 * Runs the full agent pipeline for one hypothesis.
 * trigger: "user" (someone clicked Analyze) or "cron" (scheduled re-run).
 * Throws on failure; errors from the synthesis call carry .status and .details.
 */
export async function analyzeHypothesis(hypothesis, { trigger = "user" } = {}) {
  const isScheduled = trigger === "cron";
  const label = isScheduled ? "[Scheduled] " : "";
  ...
  // Only user-initiated runs add to / reorder the tracked list, so the
  // scheduled job never changes which hypotheses are being tracked.
  if (!isScheduled) {
    const TRACKED_CAP = 8;
    try {
...
object: {"queries": ["...", "..."]}`,
  ...
  await logActivity(
    `${label}Agent planned ${plannedQueries.length} search${plannedQueries.length > 1 ? "es" : ""} for "${hypothesis}": ${plannedQueries.join(" | ")}`
  );
  ...
  await logActivity(
    `${label}Queried live evidence for "${hypothesis}" (${sources.length} sources)`
  );
  ...
  try {
    const gapCheck = await callFastJSON(
...
{"needsMoreEvidence": true|false, "followUpQuery": "<specific query>" or null}`,
  ...
  if (followUpQuery) {
    await logActivity(`${label}Agent requested additional evidence: "${followUpQuery}"`);
    try {
...
`;
  ...
  if (!nemotronResponse.ok) {
    ...
    const err = new Error("Nemotron analysis failed.");
    err.status = nemotronResponse.status;
    err.details = nemotronData;
    throw err;
  }
  ...
  await logActivity(
    `${label}Hypothesis updated: "${hypothesis}" ${startingConfidence}% → ${newConfidence}%`
  );
